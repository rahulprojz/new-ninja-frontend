  ...
  if (req.body.roles) {
    Role.find({ name: { $in: req.body.roles } }, (err, roles) => {
      if (err) {
        return res
          .status(404)
          .send({ message: err, status: config.RES_STATUS_FAIL });
      }
      ...
      user.save(async (err, nUser) => {
        if (err) {
          return res.status(500).send({
            message: `E11000 duplicate key error collection: users index: email_1 dup key: { email: ${req.body.email}}`,
            status: config.RES_STATUS_FAIL,
          });
        }
...
exports.verifyEmail = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ message: "An error occured", status: config.RES_STATUS_FAIL });
  }
};
...
exports.forgot = async (req, res) => {
  ...
  User.findOne({ email: req.params.email }, {}, async function (err, user) {
    ...
    try {
      ...
      return res
        .status(200)
        .send({ resettoken: token, status: config.RES_STATUS_SUCCESS });
    } catch {
      return res.status(205000).send({
        message: "There is no user with this email.",
        status: config.RES_STATUS_FAIL,
      });
    }
...
exports.requestEmailVerify = (req, res) => {
  User.findOne({
    ...
    if (!user) {
      return res
        .status(404)
        .send({ message: "Not exist user", status: config.RES_STATUS_FAIL });
    }
  ...
  User.findOne({
    ...
    .exec(async (err, user) => {
      ...
      if (!user) {
        return res
          .status(404)
          .send({ message: "Incorrect token", status: config.RES_STATUS_FAIL });
      }
